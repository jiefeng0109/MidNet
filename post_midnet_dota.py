...
def post():
    ...
    con_thres = 0.06
    files = os.listdir('exp/midnet_log/result/')
    with open('/data/DOTA/COCO/annotations/val.json', 'r', encoding='utf8') as f:
        gt = json.load(f)
    ...
    for file_id, file_name in enumerate(files):
        res_path = 'exp/midnet_no_sdcn/result/'
        img_path = '/data/DOTA/COCO/val/'
        img = Image.open(img_path + file_name[:-4] + '.png')
        ...
        for cat_id in range(1, 16):
            ...
            for c in center:
                ...
                obj[0] = [cx, cy, c[4], r]
                objs.append(obj)
                # draw.ellipse(((cx - 3, cy - 3), (cx + 3, cy + 3)), fill=tuple(color), outline=tuple(color), width=3)
            ...
            for p in left:
                ...
                # draw.ellipse(((x - 3, y - 3), (x + 3, y + 3)), fill=tuple(color), outline=tuple(color), width=3)
                # draw.line(((x, y), (x + w, y - h)), fill=tuple(color), width=1)
            ...
            for obj in objs:
                ...
                cot = 0
                for p in obj[1:5]:
                    if p[2] <= 1 or p[3] <= 1 or 1 / 256 > abs(p[2] / p[3]) or abs(p[3] / p[3]) > 256:
        ...
        img.save('midnet_log/bboxes/' + file_name[:-4] + '.png', quality=100)
        # print('midnet_log/bboxes/' + file_name[:-4] + '.png')
