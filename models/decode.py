...
def ctdet_decode(hms, whs, K=100):
    batch, cat, height, width = hms[0].size()

    # perform nms on heatmaps
    temp = []
    for heat in hms:
        temp.append(_nms(heat))
    hms = temp

    detections = []
    scores, inds, clses, ys, xs = _topk(hms[0], K=K)
    xs = xs.view(batch, K, 1) + 0.5
    ys = ys.view(batch, K, 1) + 0.5
    wh = _transpose_and_gather_feat(whs[0], inds)
    wh = wh.view(batch, K, 1)
    clses = clses.view(batch, K, 1).float()
    scores = scores.view(batch, K, 1)
    bboxes = torch.cat([xs, ys, wh[..., 0:1] * 0.8], dim=2)
    detections.append(torch.cat([bboxes, scores, clses], dim=2))

    for hm, wh in zip(hms[1:5], whs[1:5]):
        ...
        clses = clses.view(batch, K, 1).float()
        scores = scores.view(batch, K, 1)
        bboxes = torch.cat([xs, ys, wh[..., 0:1], wh[..., 1:2]], dim=2)
        detections.append(torch.cat([bboxes, scores, clses], dim=2))

    return detections, hms
